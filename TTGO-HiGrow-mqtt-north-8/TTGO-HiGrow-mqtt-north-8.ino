...
// Change to the appropiate network credentials
...
const String device_name = "North_8";
const String device_placement = "NorthWindow";

#define uS_TO_S_FACTOR 1000000ULL  //Conversion factor for micro seconds to seconds
#define TIME_TO_SLEEP  3600       //Time ESP32 will go to sleep (in seconds), can be changed as wanted.
...
const char broker[] = "192.168.xxx.xxx";
int        port     = 1883;
...
void setup() {
  ...
  timeClient.setTimeOffset(7200);  // For some reason, the daylightOffset_sec do not work, so I have changed the offset my self (2 hours in this case)
  ...
  float bat = readBattery();
    if (bat >= 4) bat = 100;
    else if (bat >= 3.9) bat = 75;
    else if (bat >= 3.8) bat = 50;
    else if (bat >= 3.7) bat = 25;
    else if (bat >= 3.6) bat = 20;
    else if (bat < 3.6) bat = 10;
  config.bat = bat;    
